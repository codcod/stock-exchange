...
**Risk Engine** - The lifespan hook fetches all accounts from the Account service (`GET /accounts`, matching the "Account and Risk Engine freshness" section above) and loads all instruments from `risk_engine.instruments` into its in-memory caches.

Note: The matching engine reads from `order_management.orders` directly — a cross-schema, same-Postgres-instance read, not a service call. The risk engine's account warm-up, by contrast, is an HTTP call to the Account service, not a database read.
